...
class AuthController extends Controller
{
    ...
    {
        $admin = Admin::firstWhere('email', $request->input('username'));
        if ($admin) {
        ...
        $guru = Guru::firstWhere('email', $request->input('username'));
        if ($guru) {
            ...
            if (Hash::check($request->input('password'), $guru->password)) {
                ...
                // Cek level akun dan redirect ke halaman sesuai level.
                if ($guru->role == 2) {
                    return redirect()->intended('/kaprog')->with('pesan', "
                        <script>
                            swal({
                                title: 'Berhasil!',
                                text: 'login berhasil',
                    ...
                    // Jika level tidak sesuai, Anda dapat mengarahkan pengguna ke halaman yang sesuai atau memberikan pesan kesalahan.
                    return redirect('/')->with('pesan', "
                        <script>
                            swal({
                                title: 'Login Failed!',
                                text: 'Level akun tidak valid',
                                type: 'error',
                                padding: '2em'
                            })
                        </script>
        ...
        $siswa = Siswa::firstWhere('nisn', $request->input('username'));
        if ($siswa) {
        ...
        $dudi = DuDi::firstWhere('email', $request->input('username'));
        if ($dudi) {
            ...
            if (Hash::check($request->input('password'), $dudi->password)) {
                $request->session()->put('dudi', $dudi);
                return redirect()->intended('/dudi')->with('pesan', "
                    <script>
                        swal({
                            title: 'Berhasil!',
                            text: 'login berhasil',
