            ...
            <div class="col-lg-6 layout-spacing">
                <div class="widget shadow">
                    <div class="widget-content-area">
                        <h5 class="">{{ $km_tugas_siswa->km_tugas->nama_tugas }}</h5>
                        <table cellpadding="2">
                            <tr>
                                <th>Siswa</th>
                                <th> : {{ $km_tugas_siswa->siswa->nama_siswa }}</th>
                            </tr>
                            <tr>
                                <th>Waktu dikirim</th>
                                <th> : {{ $km_tugas_siswa->date_send }}</th>
                            </tr>
                            <tr>
                                <th>Status</th>
                                <td> : {!! ($km_tugas_siswa->is_telat == 1) ? '<span class="badge badge-danger">Terlambat</span>' : '<span class="badge badge-success">Sukses</span>' !!}</td>
                            </tr>
                            <tr>
                                <th>Nilai</th>
                                <th> : {!! ($km_tugas_siswa->nilai === null) ? 'Belum dinilai' : '<span class="text-primary">' . $km_tugas_siswa->nilai . '/100 Point </span>' !!}</th>
                            </tr>
                        </table>
                        <table class="mt-2">
                            <tr>
                        ...
                        {!! ($km_tugas_siswa->catatan_guru === null) ? '<p>Tidak ada</p>' : '<p>'. $km_tugas_siswa->catatan_guru .'</p>' !!}
                        <hr>
                        <p style="">
                            {{ $km_tugas_siswa->teks }}
                        </p>

                        @if ($km_tugas_siswa->file !== null)
                            <div class="row">
                        ...
                        <form action="{{ url("/guru/km_nilai_tugas/" . $km_tugas_siswa->id . '/' . $km_tugas_siswa->kode) }}" method="POST">
                            @csrf
                        ...
                        <a href="{{ url("/guru/km_tugas/" . $km_tugas_siswa->kode) }}" class="btn btn-danger btn-sm mt-3"><span data-feather="arrow-left-circle"></span> Kembali</a>
                    </div>
                </div>
            </div>
        </div>
